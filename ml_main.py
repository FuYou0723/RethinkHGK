...
def main(cfg: DictConfig):
    ...
    data_root = os.path.join(cfg.data.root,'data')
    x_list, y_list, meta = load_data(
        cfg.data.name, data_root, cfg.data.degree_as_tag, model_type
    )
    ...
    if cfg.model.name == "graph_subtree":
        ...
        model = HypergraphSubtreeKernel(normalize=cfg.model.normalize,n_iter=4,_oa=True)
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(n_fold_idx):
        ...
        if cfg.model.name in ['hypergraph_sck','hypergraph_spk','hypergraph_svk']:
            ...
            ft_path=f'{cfg.data.root}/models/H-motifs/{cfg.data.name}_results.pickle'
            ...
            ft_path=f'{cfg.data.root}/models/MoCHy/{cfg.data.name}_results.pickle'
            model._load_ft(ft_path)
            K_train = model.fit_transform(train_idx).cpu().numpy()
            K_test = model.transform(test_idx).cpu().numpy()
    ...
    print(f"mode: {cfg.model.name} | data:{cfg.data.name}")
    print("--------------------------------------------------")
    return test_res
